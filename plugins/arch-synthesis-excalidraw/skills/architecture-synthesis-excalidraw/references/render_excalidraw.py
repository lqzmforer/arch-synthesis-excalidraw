...
Usage:
    cd <plugin-root>/skills/architecture-synthesis-excalidraw/references
    uv sync
...
def render(excalidraw_path: Path, output_path: Path | None = None, scale: int = 2, max_width: int = 1920) -> Path:
    try:
        ...
        print("Run: cd <plugin-root>/skills/architecture-synthesis-excalidraw/references && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd <plugin-root>/skills/architecture-synthesis-excalidraw/references && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
